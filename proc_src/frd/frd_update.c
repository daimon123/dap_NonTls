...
int frd_UpdateExternalByHwBaseTb(_DAP_AGENT_INFO *p_AI, int *hbExt)
{
    ...
    if(rowCnt > 0)
    {
        snprintf	(sqlBuf, sizeof(sqlBuf), "update HW_BASE_TB set "
    ...
    {
        snprintf	(sqlBuf, sizeof(sqlBuf), "insert into HW_BASE_TB "
                           "("
                           "US_SQ,"
                           "HB_UNQ,"
                           "HB_AGENT_VER,"
...
int frd_UpdateAccessByHwBaseTb(_DAP_AGENT_INFO *p_AI, char *p_prefix, char *p_postfix, int* hbExt)
{
    ...
    rxt = frd_CheckChangeBaseinfo(p_AI->user_key,
                                    p_AI->user_ip,
                                    p_AI->user_mac,
                                    p_AI->hb_del,
                                    p_AI->hb_external,
    ...
    *hbExt = resExt;
    ...
    /** Agent가  전송한 HW_BASE_TB 테이블의 HB_DEL컬럼은 0으로 무조건 Update **/
    /*if(p_AI->hb_del != 0)
    {
        memset(szStrTemp, 0x00, sizeof(szStrTemp));
        if(CommaFlag == 0x00)
    ...
    if(CommaFlag == 0x00)
    {
        snprintf(szStrTemp, sizeof(szStrTemp)," HB_DISABLE=0");
    }
    else
    {
        snprintf(szStrTemp, sizeof(szStrTemp),", HB_DISABLE=0");
...
int frd_UpdateUserTb(_DAP_AGENT_INFO *p_AI, char *p_prefix, char *p_postfix)
{
    ...
    if(p_AI->user_ip[0] != 0x00)
    {
        rxt = frd_CheckChangeUserinfo(p_AI->user_seq, p_AI->user_ip);
        if(rxt > 0 )
        {
            isChange = 1;
            ...
            WRITE_INFO(CATEGORY_DB, "Change user sq(%llu)ip(%s)", p_AI->user_seq, p_AI->user_ip);
