...
//    char				cmdSyslog[256] = {0x00,};
   ...
   */
    ...
    rxt = frd_selectHbsqByHwBaseTb(p_EP->user_key, &hbSq);
    if(rxt <= 0)
    {
    ...
    rxt = frd_SelectEvsqEventTb(hbSq, p_EP->ev_type, &evSq);
...
//               || (p_EP->ev_type == PROCESS_BLACK      && p_stLen == -1) /* Process Black [off]일때는 Event 삭제 안하도록 예외처리 */
        ...
        else //해지가 아니라면 동일한 데이터가 또 들어옴
        {
            ...
            // syslog 이벤트 cfg check하여 y인 경우 우리 syslog 프로세스 실행.
            fcom_GetProfile("EVENT_LOGGING","USE_FLAG", local_szSyslogFlag,"N");
            if ( local_szSyslogFlag[0] == 'y' || local_szSyslogFlag[0] == 'Y' )
            {
                // p_EP->user_ip(Client IP), p_Ep->ev_level(이벤트 레벨), p_EP->ev_type(이벤트 타입)
                frd_ExecSyslog(p_EP->user_ip, p_EP->ev_level, p_EP->ev_type);
            }
                        ...
                        char *p_postfix)
{
    ...
    strPrevHistSq = frd_GetPrevHistSqByBase(	"HW_BASE_TB",
                        ...
                        char 				*p_postfix)
{
    ...
    if(rxt > 0)
    {
        ...
        strPrevHistSq = frd_GetPrevHistSq("SYSTEM_TB", "ST_CURR_HIST_SQ,ST_CURR_HIST_DATE", hbSq);
...
//    }
    ...
    if( fcom_malloc((void**)&arrInstalledVm, sizeof(char) * (VM_LEN * p_ST->st_installed_vm_size) ) != 0)
    {
        WRITE_CRITICAL(CATEGORY_DEBUG,"fcom_malloc Failed " );
        fcom_MallocFree((void**)&strPrevHistSq);
        return (-1);
...
)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("CONNECT_EXT_TB", "CE_CURR_HIST_SQ,CE_CURR_HIST_DATE", hbSq);
    ...
    for(idx=0; idx<p_CE->size; idx++)
    {
        ...
        // update currHistSq to tb
        memset  (sqlBuf, 0x00, sizeof(sqlBuf));
        sprintf (sqlBuf, "update CONNECT_EXT_TB set CE_CURR_HIST_SQ=%llu where CE_SQ=%llu",
                 currHistSq,tbSq);
        ...
        char 			      *p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("OS_TB", "OS_CURR_HIST_SQ,OS_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset	(sqlBuf, 0x00, sizeof(sqlBuf));
        ...
        char* 				p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("CPU_TB", "CU_CURR_HIST_SQ,CU_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset(sqlBuf, 0x00, sizeof(sqlBuf));
        ...
        char*				p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("NET_ADAPTER_TB", "NA_CURR_HIST_SQ,NA_CURR_HIST_DATE", hbSq);
        ...
        char*				p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("WIFI_TB", "WF_CURR_HIST_SQ,WF_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset	(sqlBuf, 0x00, sizeof(sqlBuf));
        ...
        char*				p_postfix )
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("BLUETOOTH_TB", "BT_CURR_HIST_SQ,BT_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset	(sqlBuf, 0x00, sizeof(sqlBuf));
        ...
        char*				p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("NET_CONNECTION_TB", "NC_CURR_HIST_SQ,NC_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset	(sqlBuf, 0x00, sizeof(sqlBuf));
        ...
        char*				p_resVal)
{
    ...
    *p_dkNewCnt = frd_SelectCountNewByDisk(hbSq, p_DK, p_resVal);
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("DISK_TB", "DK_CURR_HIST_SQ,DK_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset	(sqlBuf, 0x00, sizeof(sqlBuf));
        ...
        char*				p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("NET_DRIVE_TB", "ND_CURR_HIST_SQ,ND_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset	(sqlBuf, 0x00, sizeof(sqlBuf));
        ...
        char*				p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("NET_PRINTER_TB", "NP_CURR_HIST_SQ,NP_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset	(sqlBuf, 0x00, sizeof(sqlBuf));
        ...
        char*				p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("NET_SCAN_TB", "NS_CURR_HIST_SQ,NS_CURR_HIST_DATE", hbSq);
    }
        ...
        char*				p_postfix)
{
    ...
    for(idx=0; idx<p_WD->size; idx++)
    {
        ...
        // 이전 DV_SQ가 존재하면, 존재하는 장비정보인 경우
        if(rowCnt > 0)
        {
            // 현재 WIN_DRV_TB 테이블에서 DV_CURR_HIST_SQ 데이터를 가져와서 History 테이블의 이전 PREV_HIST_SQ값에 넣어준다.
            char *strPrevHistSq = frd_GetPrevHistSqWinDrv(	"WIN_DRV_TB",
...
//    char               local_szCurrHistSq[MAX_WINDRV_COUNT][30 +1] = { {0x00}, };

    char				sqlPath[127 +1]         = {0x00,};
    char				sqlClassDesc[127 +1]    = {0x00,};
    char				sqlDesc[127 +1]         = {0x00,};
    char				sqlDriver[127 +1]       = {0x00,};
    char				sqlName[127 +1]         = {0x00,};
    char				strCurrHistDate[8 +1]   = {0x00,};
    char				sqlBuf[4095 +1]         = {0x00,};
//    char				histTableName[50 +1]    = {0x00,};
...
//    fdb_GetHistoryTableName("WIN_DRV_TB", histTableName, p_prefix, p_postfix);
    ...
    /*
    // 2022.02.26 기업은행 WIN_DRV DB 성능이슈로 주석처리.
    // 현재 매니저에서 WIN_DRV History는 사용하지 않는다.
    // 추후 사용시 고려필요.
    if ( g_stProcFrdInfo.nWinDrvHistFlag == 1 ) //WIN_DRV_HISTORY 데이터 처리 여부. 0 : 미처리, 1 : 처리
    {
        // 루프를 돌며 기존에 존재하는 데이터인경우 DAP_DB의 CURR_HIST_SQ값을 가져온다.
        for(idx = 0; idx < p_WD->size; idx++)
        {
            memset(sqlDriver, 0x00, sizeof(sqlDriver));
            fcom_ReplaceAll(p_WD->WinDrvValue[idx].dv_driver, "\\", "\\\\", sqlDriver);
            ...
            memset( local_szCurrHistSq[idx], 0x00, sizeof(local_szCurrHistSq[idx]) );
            rowCnt = fdb_SelectSqWinDrv( hbSq,
                                         sqlName,
                                         p_WD->WinDrvValue[idx].dv_service,
                                         sqlDriver,
            ...
            // 이전 DV_SQ가 존재하면, 존재하는 장비정보인 경우 메모리에 CURR_HIST_SQ값 저장.
            if ( rowCnt > 0 )
            {
                char *strPrevHistSq = frd_GetPrevHistSqWinDrv(	"WIN_DRV_TB",
                                                                  "DV_CURR_HIST_SQ,DV_CURR_HIST_DATE",
                                                                  tbSq);
                snprintf(local_szCurrHistSq[idx], sizeof(local_szCurrHistSq[idx]), "%s", strPrevHistSq);
    ...
    */
    ...
    for ( idx = 0; idx < p_WD->size; idx++ )
    {
        // dv_class_reset 값을 검사하여 1이면 해당 장비번호의 class를 삭제한다.
        if ( p_WD->WinDrvValue[idx].dv_class_reset == 1 )
        {
            memset	(sqlBuf, 0x00, sizeof(sqlBuf));
            snprintf	(sqlBuf, sizeof(sqlBuf),
                         "delete from WIN_DRV_TB where HB_SQ=%llu AND DV_CLASS='%s'",
                         hbSq, p_WD->WinDrvValue[idx].dv_class);
...
//        tbSq = fdb_SqlInsertId(g_stMyCon);
        ...
        // History Insert, WIN_DRV_HISTORY 데이터 처리 여부. 0 : 미처리, 1 : 처리
       /*
        if ( local_szCurrHistSq[idx][0] != 0x00 && g_stProcFrdInfo.nWinDrvHistFlag == 1)
        {
            memset	(sqlBuf, 0x00, sizeof(sqlBuf));
            snprintf	(sqlBuf, sizeof (sqlBuf),
                         "insert into %s ("
        ...
        else // 기존 데이터의 CURR_HIST_SQ가 없는경우
        {
            if (g_stProcFrdInfo.nWinDrvHistFlag == 1) //WIN_DRV_HISTORY 데이터 처리 여부. 0 : 미처리, 1 : 처리
            {
                fdb_InsertHistory(tbSq, "WIN_DRV_TB", histTableName);
            }
        }
        */
    ...
    WRITE_DEBUG(CATEGORY_DB, "TABLE [ WIN_DRV_TB / WIN_DRV_TB_HISTORY ] INSERT : %d, DELETE : %d ", insertCnt, deleteCnt);
        ...
        char*				p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("RDP_SESSION_TB", "RDP_CURR_HIST_SQ,RDP_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset	(sqlBuf, 0x00, sizeof(sqlBuf));
        ...
        char*				p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("OS_ACCOUNT_TB", "OA_CURR_HIST_SQ,OA_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset	(sqlBuf, 0x00, sizeof(sqlBuf));
        ...
        char*				p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("SHARE_FOLDER_TB", "SF_CURR_HIST_SQ,SF_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset	(sqlBuf, 0x00, sizeof(sqlBuf));
        ...
        char*				p_postfix )
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("INFRARED_DEVICE_TB", "ID_CURR_HIST_SQ,ID_CURR_HIST_DATE", hbSq);
        ...
        char*				p_postfix )
{
    ...
    char				*strPrevHistSq = NULL;
    char				strCurrHistDate[8 +1] = {0x00,};
    char				sqlPath[255 +1] = {0x00,};
    char				sqlBuf[4095 +1] = {0x00,};
//    char				sqlSummary[1280 +1] = {0x00,};
    char				sqlSummary[4096 +1] = {0x00,};
    char				histTableName[50 +1] = {0x00,};
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("PROCESS_TB", "PS_CURR_HIST_SQ,PS_CURR_HIST_DATE", hbSq);
        ...
        char*				p_postfix )
{
    ...
    if(rxt > 0)
    {
        ...
        strPrevHistSq = frd_GetPrevHistSq("ROUTER_TB", "RT_CURR_HIST_SQ,RT_CURR_HIST_DATE", hbSq);
        // 2. delete tb
        memset	(sqlBuf, 0x00, sizeof(sqlBuf));
                          ...
                          char*				    p_postfix)
{
    ...
    if(rxt > 0)
    {
        // If existing data exists
        // 1. get curr_hist_sq from tb
        strPrevHistSq = frd_GetPrevHistSq("CTRL_PROCESS_CPU_TB", "CP_CURR_HIST_SQ,CP_CURR_HIST_DATE", hbSq);
        ...
        WRITE_INFO(CATEGORY_INFO, "Succeed in delete, rowCnt(%d)hbSq(%llu)", rowCnt, hbSq);
...
{
    int     local_nInsertcnt            = 0;
    char    local_szTableName[63 +1]    = {0x00,};
    char    local_szDataBaseName[31 +1] = {0x00,};
    char    sqlBuf[512 +1] = {0x00,};
    char    local_szTimeBuffer[14 +1] = {0x00,};
    char    local_szYYYY[4 +1]   = {0x00,};
    char    local_szMM[2 +1]     = {0x00,};


    /** Get History Database Time **/
    memset(local_szTimeBuffer, 0x00, sizeof(local_szTimeBuffer));
    fcom_GetTime(local_szTimeBuffer, 0);

    memcpy(local_szYYYY, local_szTimeBuffer, 4);
    memcpy(local_szMM, &local_szTimeBuffer[4], 2);

    WRITE_DEBUG(CATEGORY_DEBUG,"Get History Time : [%s]  YYYY : [%s], MM : [%s]", local_szTimeBuffer, local_szYYYY, local_szMM);

    snprintf(local_szDataBaseName, sizeof(local_szDataBaseName),"%s_%s", "DAP_HISTORY", local_szYYYY);
    snprintf(local_szTableName, sizeof(local_szTableName),"%s_%s", "BASE_STATUS_HISTORY", local_szMM);

    WRITE_DEBUG(CATEGORY_DEBUG,"History Table Name : [%s.%s] ", local_szDataBaseName, local_szTableName);

    /** Set Query **/
    memset(sqlBuf, 0x00, sizeof(sqlBuf));
    snprintf(sqlBuf, sizeof(sqlBuf),
             "insert INTO %s.%s (HB_SQ, ONOFF_STATUS, STATUS_TIME) "
             "VALUES(%s,'%s',sysdate()) ",
             local_szDataBaseName, local_szTableName,
             param_HbSq,
             param_AgentStatus);


    local_nInsertcnt = fdb_SqlQuery2(g_stMyCon, sqlBuf);
    if(g_stMyCon->nErrCode != 0)
    {
        WRITE_CRITICAL(CATEGORY_DB,"Fail in query, errcode(%d) msg(%s)",
                       g_stMyCon->nErrCode,
    ...
    WRITE_INFO(CATEGORY_DB, "BASE_STATUS_HISTORY Proc insertCnt(%d) ", local_nInsertcnt);
