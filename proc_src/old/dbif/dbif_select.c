...
char* fdbif_GetPrevHistSq(char *p_tableName, char *p_colName, unsigned long long p_Sq)
{
    ...
    if(rowCnt > 0)
    {
        ...
        while(fdb_SqlFetchRow(g_stMyCon) == 0)
        {
            ...
            {
                if(loop == 0)	sprintf(result, "%llu", atoll(g_stMyCon->row[0]));
                else			sprintf(result+strlen(result), ",%llu", atoll(g_stMyCon->row[0]));
...
)
{
    ...
    for(idx=0; idx < p_NA->size; idx++)
    {
        ...
        if(tokenCnt > 0)
        {
            ...
            {
                memset(sqlBuf, 0x00, sizeof(sqlBuf));
                snprintf(sqlBuf, sizeof(sqlBuf),"select HB_SQ from HW_BASE_TB where %s='%s'", sqlColumn,arrStr[i]);
                ...
                else if(rowCnt == 1) //자신이IP를바꾼것
                {
                    ...
                    WRITE_INFO(CATEGORY_DB,	"- hbAccessRes(%s)arrStr[%d](%s) ",
                               hbAccessRes,i,arrStr[i] );
                ...
                else
                {
                    ...
                    WRITE_INFO(CATEGORY_DB,	"Maybe ip changed, arrStr(%s)",
                               arrStr[i]);
                }
        ...
        else /* 요청받은 MAC주소 및 IP주소가 한개인경우 */
        {
            ...
            else if(rowCnt == 1) //HW_BASE_TB에 이미 존재하는 IP로 자신의 IP를변경한 경우
            {
                ...
                WRITE_INFO(CATEGORY_DB,	"- hbAccessRes(%s)naValue(%s)",
                           hbAccessRes,naValue );
