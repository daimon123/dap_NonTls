...
void fmaster_RestartProcess(void) {
    ...
    char    szDapHome[128] = {0x00,};
    char    local_szPidPath[256] = {0x00,};
    char*   local_ptrProcessName = NULL;
    RestartFlag = 0x01;

    snprintf(szDapHome, sizeof(szDapHome), "%s", getenv("DAP_HOME"));
    snprintf(szCommand, sizeof(szCommand), "%s", "pkill -9 dap_pcif");
    WRITE_INFO(CATEGORY_DEBUG,"Restart Process [%s] \n", szCommand);
    system(szCommand);

    snprintf(szCommand, sizeof(szCommand), "%s", "pkill -9 dap_proxy");
    WRITE_INFO(CATEGORY_DEBUG,"Restart Process [%s] \n", szCommand);
    system(szCommand);

    snprintf(szCommand, sizeof(szCommand), "%s", "pkill -9 dap_frd");
    WRITE_INFO(CATEGORY_DEBUG,"Restart Process [%s] \n", szCommand);
    system(szCommand);

    snprintf(szCommand, sizeof(szCommand), "%s", "pkill -9 dap_policy_fw");
    WRITE_INFO(CATEGORY_DEBUG,"Restart Process [%s] \n", szCommand);
    system(szCommand);
    ...
    snprintf(szCommand, sizeof(szCommand), "%s/bin/%s", szDapHome, "dap_pcif pcif &");
    system(szCommand);
    snprintf(szCommand, sizeof(szCommand), "%s/bin/%s", szDapHome, "dap_proxy proxy &");
    system(szCommand);
    snprintf(szCommand, sizeof(szCommand), "%s/bin/%s", szDapHome, "dap_frd frd &");
    system(szCommand);
    snprintf(szCommand, sizeof(szCommand), "%s/bin/%s", szDapHome, "dap_policy_fw fw &");
    system(szCommand);
...
int fmaster_InvokeProcess(_PROCESS_INFO* pInfo)
{
    ...
    for(i = 0; i < gProcess; i++)
    {
        ...
        if(pInfo[i].status == 'K' && access(cpProcName, X_OK) == 0 && pInfo[i].autoflag == 'A')
        {
            ...
            WRITE_INFO(CATEGORY_DB,"Invoke Process name %s ", pInfo[i].pname);
...
{
    const int MaxCheckCnt = 20;
    int i = 0;
    char buf[512 +1];
    char szFilePath[256] = {0x00,};
    char szDtPath[128] = {0x00,};
    ...
    snprintf(szDtPath, sizeof(szDtPath),
             "%s/data/dt/", getenv("DAP_HOME") );
    snprintf( szFilePath, sizeof(szFilePath),
              "%s%s", szDtPath, "DumpFlag" );

    // File Check
    for ( i = 0; i < MaxCheckCnt; i++) {
        // FRD dump 작업중 SEGV 발생한 경우 재 시도
        if ( fcom_fileCheckStatus(szFilePath) == 0){
            WRITE_CRITICAL(CATEGORY_DEBUG,"frd dump Detect Segv Retry %d,cmd: %s, file : %s",i,buf,szFilePath );
            system(buf);
        } else {
            break;
        }
    ...
    if ( fcom_fileCheckStatus(szFilePath) == 0) {
        unlink(szFilePath);
        WRITE_CRITICAL(CATEGORY_DEBUG,"delete frd dump Detect file %s",szFilePath );
    }
}
