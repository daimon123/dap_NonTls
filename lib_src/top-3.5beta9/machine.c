...
get_system_info (struct system_info *si)
{
  ...
  (void) getkval (maxmem_offset, (int *) (&maxmem), sizeof (maxmem), "maxmem");
   ...
   if (kd != NULL)		/* always get freemem from kvm if we can*/
	...
	//int nFreeMemory  = (int)(nFreeMemory * (float)(((float)nPageSize/1024.0)/1024.0));
    //int nTotalMemory = (int)(nTotalMemory * (float)(((float)nPageSize/1024.0)/1024.0))
	//LogRet("-----> nTotalMemory[%d]\n", nTotalMemory);
	//LogRet("-----> nFreeMemory[%d]\n", nFreeMemory);
