CREATE OR REPLACE VIEW {VW_SYSTEM_HISTORY} as 
select 
st.*,
...
from {SYSTEM_HISTORY} as st 
    ...
    ) as info on st.HB_SQ = info.HB_SQ 
left join DAP_DB.EVENT_TB as ev on st.HB_SQ = ev.HB_SQ 
	and ev.EV_TYPE in ({EV_TYPE}) 
	and ev.EV_EXIST = 1 
	and st.ST_DETECT_TIME = ev.EV_DETECT_TIME 
left join {EVENT_HISTORY} as evh on st.HB_SQ = evh.HB_SQ 
	and evh.EV_TYPE in ({EV_TYPE}) 
	and ((evh.EV_EXIST = 2 and st.ST_DETECT_TIME = evh.EV_DETECT_TIME) 
		or (evh.EV_EXIST = 0 and st.ST_DETECT_TIME = evh.EV_DUP_DETECT_TIME)) 
