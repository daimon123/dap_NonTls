CREATE OR REPLACE VIEW {VW_INFRARED_DEVICE_HISTORY} as 
select 
id.*,
...
evh.EVH_SQ,
evh.EVH_RECORD_TIME 
from {INFRARED_DEVICE_HISTORY} as id 
    left join (
    ...
    ) as info on id.HB_SQ = info.HB_SQ 
left join DAP_DB.EVENT_TB as ev on id.HB_SQ = ev.HB_SQ 
	and ev.EV_TYPE in ({EV_TYPE}) 
	and ev.EV_EXIST = 1 
	and id.ID_DETECT_TIME = ev.EV_DETECT_TIME 
left join {EVENT_HISTORY} as evh on id.HB_SQ = evh.HB_SQ 
	and evh.EV_TYPE in ({EV_TYPE}) 
	and ((evh.EV_EXIST = 2 and id.ID_DETECT_TIME = evh.EV_DETECT_TIME) 
		or (evh.EV_EXIST = 0 and id.ID_DETECT_TIME = evh.EV_DUP_DETECT_TIME)) 
