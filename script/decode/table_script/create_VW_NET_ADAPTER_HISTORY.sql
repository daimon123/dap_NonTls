CREATE OR REPLACE VIEW {VW_NET_ADAPTER_HISTORY} as 
select 
na.*,
...
evh.EVH_SQ,
evh.EVH_RECORD_TIME 
from {NET_ADAPTER_HISTORY} as na 
    left join (
    ...
    ) as info on na.HB_SQ = info.HB_SQ 
left join DAP_DB.EVENT_TB as ev on na.HB_SQ = ev.HB_SQ 
	and ev.EV_TYPE in ({EV_TYPE}) 
	and ev.EV_EXIST = 1 
	and na.NA_DETECT_TIME = ev.EV_DETECT_TIME 
left join {EVENT_HISTORY} as evh on na.HB_SQ = evh.HB_SQ 
	and evh.EV_TYPE in ({EV_TYPE}) 
	and ((evh.EV_EXIST = 2 and na.NA_DETECT_TIME = evh.EV_DETECT_TIME) 
		or (evh.EV_EXIST = 0 and na.NA_DETECT_TIME = evh.EV_DUP_DETECT_TIME)) 
