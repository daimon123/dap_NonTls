CREATE OR REPLACE VIEW {VW_NET_DRIVE_HISTORY} as 
select 
nd.*,
...
from {NET_DRIVE_HISTORY} as nd 
    ...
    ) as info on nd.HB_SQ = info.HB_SQ 
left join DAP_DB.EVENT_TB as ev on nd.HB_SQ = ev.HB_SQ 
	and ev.EV_TYPE in ({EV_TYPE}) 
	and ev.EV_EXIST = 1 
	and nd.ND_DETECT_TIME = ev.EV_DETECT_TIME 
left join {EVENT_HISTORY} as evh on nd.HB_SQ = evh.HB_SQ 
	and evh.EV_TYPE in ({EV_TYPE}) 
	and ((evh.EV_EXIST = 2 and nd.ND_DETECT_TIME = evh.EV_DETECT_TIME) 
		or (evh.EV_EXIST = 0 and nd.ND_DETECT_TIME = evh.EV_DUP_DETECT_TIME)) 
