CREATE OR REPLACE VIEW {VW_NET_PRINTER_HISTORY} as 
select 
np.*,
...
evh.EVH_SQ,
evh.EVH_RECORD_TIME 
from {NET_PRINTER_HISTORY} as np 
    left join (
    ...
    ) as info on np.HB_SQ = info.HB_SQ 
left join DAP_DB.EVENT_TB as ev on np.HB_SQ = ev.HB_SQ 
	and ev.EV_TYPE in ({EV_TYPE}) 
	and ev.EV_EXIST = 1 
	and np.NP_DETECT_TIME = ev.EV_DETECT_TIME 
left join {EVENT_HISTORY} as evh on np.HB_SQ = evh.HB_SQ 
	and evh.EV_TYPE in ({EV_TYPE}) 
	and ((evh.EV_EXIST = 2 and np.NP_DETECT_TIME = evh.EV_DETECT_TIME) 
		or (evh.EV_EXIST = 0 and np.NP_DETECT_TIME = evh.EV_DUP_DETECT_TIME)) 
