CREATE OR REPLACE VIEW {VW_NET_CONNECTION_HISTORY} as 
select 
nc.*,
...
evh.EVH_SQ,
evh.EVH_RECORD_TIME 
from {NET_CONNECTION_HISTORY} as nc 
    left join (
    ...
    ) as info on nc.HB_SQ = info.HB_SQ 
left join DAP_DB.EVENT_TB as ev on nc.HB_SQ = ev.HB_SQ 
	and ev.EV_TYPE in ({EV_TYPE}) 
	and ev.EV_EXIST = 1 
	and nc.NC_DETECT_TIME = ev.EV_DETECT_TIME 
left join {EVENT_HISTORY} as evh on nc.HB_SQ = evh.HB_SQ 
	and evh.EV_TYPE in ({EV_TYPE}) 
	and ((evh.EV_EXIST = 2 and nc.NC_DETECT_TIME = evh.EV_DETECT_TIME) 
		or (evh.EV_EXIST = 0 and nc.NC_DETECT_TIME = evh.EV_DUP_DETECT_TIME)) 
