CREATE OR REPLACE VIEW {VW_CPU_HISTORY} as 
select 
cu.*,
...
from {CPU_HISTORY} as cu 
    ...
    ) as info on cu.HB_SQ = info.HB_SQ 
left join DAP_DB.EVENT_TB as ev on cu.HB_SQ = ev.HB_SQ 
	and ev.EV_TYPE in ({EV_TYPE}) 
	and ev.EV_EXIST = 1 
	and cu.CU_DETECT_TIME = ev.EV_DETECT_TIME 
left join {EVENT_HISTORY} as evh on cu.HB_SQ = evh.HB_SQ 
	and evh.EV_TYPE in ({EV_TYPE}) 
	and ((evh.EV_EXIST = 2 and cu.CU_DETECT_TIME = evh.EV_DETECT_TIME) 
		or (evh.EV_EXIST = 0 and cu.CU_DETECT_TIME = evh.EV_DUP_DETECT_TIME)) 
