//
// Created by KimByoungGook on 2023-03-28.
//

#ifndef OPENSSL_COMMON_H
#define OPENSSL_COMMON_H
...
//    WRITE_DEBUG(CATEGORY_DEBUG,"%.*s", (int)len, buf);
    WRITE_DEBUG(CATEGORY_DEBUG,"%s",buf);
}
...
void print_ssl_state()
{
    ...
    if (current_state != client.last_state) {
        if (current_state)
            WRITE_DEBUG(CATEGORY_DEBUG,"SSL-STATE: %s\n", current_state);
...
void print_ssl_error()
{
    ...
    if (len > 0)
        WRITE_DEBUG(CATEGORY_DEBUG,"SSL-ERROR: %s", buf);
    BIO_free(bio);
}
...
int do_sock_write()
{
    WRITE_DEBUG(CATEGORY_DEBUG,"@@@@@ Send : [%s] \n", client.write_buf);
